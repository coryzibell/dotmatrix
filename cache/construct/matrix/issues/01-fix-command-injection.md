...
**Status:** open
**GitHub:** [#15](https://github.com/coryzibell/matrix/issues/15)
...
From Construct review of `matrix` project. Cypher identified critical security vulnerability during security audit.
...
An attacker with write access to markdown files in `~/.matrix/ram/` could execute arbitrary commands by inserting malicious `[verify:]` directives.
...
**Recommendation:** Option B - the verification feature is useful for checking if design docs match reality, just needs guardrails.
...
- [ ] Allowlisted commands (`grep`, `find`, `test`, `cat`, `head`, `wc`) work as before
